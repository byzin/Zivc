...
int printVulkanBackendInfo(const zivc::Context& context)
{
  ...
  // Print Vulkan instance extensions
  {
    ...
    const auto& properties = backend_p->extensionPropertiesList();
  ...
  std::cout << std::endl;
  {
    ...
    const auto& properties = backend_p->layerPropertiesList();
  ...
  std::cout << std::endl;
  const auto& info_list = backend_p->deviceInfoList();

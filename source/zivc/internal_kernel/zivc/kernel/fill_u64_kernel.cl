...
#ifndef ZIVC_CL_FILL_U64_KERNEL_CL
#define ZIVC_CL_FILL_U64_KERNEL_CL
...
/*!
  ...
  */
__kernel void Zivc_fillU64Kernel(zivc::ConstGlobalPtr<uint2> data,
                                 ...
                                 const zivc::FillInfo info)
{
  ...
  for (size_t ite = 0; ite < info.batchSize(); ++ite) {
    ...
    zivc::GlobalPtr<uint2> ptr = buffer + buffer_index;
    for (size_t i = 0; i < info.dataSize(); ++i)
      ptr[i] = data[i];
  }
}

#endif // ZIVC_CL_FILL_U64_KERNEL_CL
